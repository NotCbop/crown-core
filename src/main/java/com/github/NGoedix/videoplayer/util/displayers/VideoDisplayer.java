...
public class VideoDisplayer implements IDisplay {
    ...
    private int calculateVolume(float volume, float minDistance, float maxDistance) {
        ...
        float distance = (float) pos.distance(Objects.requireNonNull(Minecraft.getInstance().player).getPosition(mc.isPaused() ? 1.0F : mc.getDeltaTracker().getGameTimeDeltaPartialTick(false)));
