...
public class TextureCache {
    ...
    private static final class FramePictureFetcher extends ImageFetch {
        public FramePictureFetcher(TextureCache cache, String originalURL) {
            super(NetworkAPI.parseURI(originalURL));

            setSuccessCallback((imageRenderer, fromCache) -> Minecraft.getInstance().executeBlocking(() -> cache.process(imageRenderer)));
            ...
            setErrorCallback((e, isVideo) -> Minecraft.getInstance().executeBlocking(() -> {
                ...
                if (!cache.isVideo()) {
                    if (e == null || e.getMessage() == null) cache.processFailed("download.exception.gif");
